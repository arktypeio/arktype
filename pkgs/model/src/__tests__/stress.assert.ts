...
const generateSpaceJson = () => {
    ...
    const defs = Object.entries(spaceSeed).reduce((result, [name, seedDef]) => {
        const variants: Record<string, any> = { [name]: seedDef }
        const randomInRange = (min: number, max: number) =>
            Math.floor(Math.random() * (max - min + 1)) + min
        const defCopyCount = 50
        for (let i = 2; i <= defCopyCount; i++) {
            variants[`${name}${i}`] = transform(seedDef as any, ([k, def]) => {
                if (typeof def === "string") {
                    for (const name in spaceSeed) {
                        def = def.replaceAll(
                            name,
    ...
    writeJson(fromHere("largeSpace.json"), defs)
}
...
describe("stress", () => {
    test("large space", () => {
        ...
        assert(space.types.user4.groups4[0].members19[0].groups48[0].title10)
            .typed as string
        // @ts-expect-error
        const invalidPropertyAccess = space.types.user4.groups4[0].members20
    })
})
