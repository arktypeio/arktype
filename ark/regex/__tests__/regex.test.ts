...
const S = regex("m?|y|a|s?si?s(p|h|a|t)")

const Z = regex("a|b|(c|d)")

const So = regex("^a?b?c?d?e?f?g?h?i?$")
//     ^?

const Sz = regex("^a?b?c?d?e?f?g?h?i?j?$")
//    ^?

//  RequiredReduction = RootDepth - MaxDepth
// reduce by 10 depth
// "a[a-z]*c" | "a[a-z]*d" | "z?[a-z]*c?"
