...
Luckily, there is a builtin API for wrapping `pipe`d functions you don't trust:
...
At this point, our implementation is starting to look pretty clean, but in many cases like this one, we can skip straight to the punch line with one of ArkType's many builtin aliases for validation and parsing, `string.json.parse`:
