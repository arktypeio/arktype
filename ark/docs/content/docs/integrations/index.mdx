...
```
important to note if you create a custom controlled input best approach to get type inference would be to pass the type to the hook itself

`const {...} = useForm<typeof User.infer>`  // Will work fine and infer the types properly
