...
An empty array or object, or an array of unit literals, can be defaulted directly in string syntax. A shared reference would leak mutations from one traversal into the next, so `= []`, `= {}`, `= ['a']`, and `= [1, 2]` assign a new array or object each time the default is applied- exactly like the thunk form they replace.

```ts
const MyObj = type({
	// each parsed value gets its own array or object
	values: "string[] = []",
	hosts: "string[] = ['localhost']",
	ports: "number[] = [3000, 8080]",
	meta: "object = {}"
})
```
...
<Callout type="info" title="Array is the only constructor that works cross-realm">

    Constructors are compared by identity, so a value created in another realm (e.g. an `<iframe>` or a `node:vm` context) will not be an `instanceof` the constructor in your realm.

    `Array` is special-cased to use `Array.isArray`, making it the only built-in constructor that will also match arrays from another realm.
