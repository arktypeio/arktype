...
An empty array or object can be defaulted directly in string syntax. A shared reference would leak mutations from one traversal into the next, so `= []` and `= {}` assign a new array or object each time the default is applied- exactly like the thunk form they replace.

```ts
const MyObj = type({
	// each parsed value gets its own array
	values: "string[] = []",
	meta: "object = {}"
})
```
