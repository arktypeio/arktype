...
Like ArkType's other [structural transformations](/docs/objects#properties-structural), `merge` builds a new object from the extracted properties, so a root narrow, filter or metadata on the original will not be preserved.
...
`merge`, `pick`, `omit`, `required`, `partial` and `map` are **transformations** rather than compositions. Instead of referencing the `Type` they're applied to, each extracts its properties and uses them to build a new object `Type` from scratch.

<Callout type="warn" title="Structural transformations extract properties and discard everything else">

    Because the result is constructed from the original's properties alone, anything attached to the object _itself_ is not carried over- most notably a root [narrow](/docs/expressions#narrow) or [filter](/docs/expressions#filter), but also [metadata](/docs/configuration#metadata) like `.describe(...)` and constraints like `.atLeastLength(...)`.
    ...
    // Type<{ name?: string; email?: string }>- the narrow is gone
    ...
    Constraints on individual properties are unaffected, since those are part of the properties being extracted. To preserve logic attached to the object, reapply it after the transformation:
    ...
    A root [morph](/docs/expressions#pipe) is the exception. Because it can't be reduced to a set of properties, transforming a `Type` like `type({ name: "string" }).pipe(user => user)` throws a `ParseError` rather than silently dropping it.
...
Like ArkType's other [structural transformations](/docs/objects#properties-structural), `pick` and `omit` build a new object from the extracted properties, so a root narrow, filter or metadata on the original will not be preserved.
...
Like ArkType's other [structural transformations](/docs/objects#properties-structural), `required` and `partial` build a new object from the extracted properties, so a root narrow, filter or metadata on the original will not be preserved.
...
Like ArkType's other [structural transformations](/docs/objects#properties-structural), `map` builds a new object from the extracted properties, so a root narrow, filter or metadata on the original will not be preserved.
