...
// Track recent hover requests to avoid duplicates
const recentHoverRequests = new Map<string, number>()
...
const getInitializedTypeScriptService = async (
	...
	try {
		...
		)._modeConfiguration.hovers = false
...
const setupMonaco = async (monaco: typeof Monaco) => {
	...
	// Add a custom hover provider to clean up type displays
	monaco.languages.registerHoverProvider("typescript", {
		provideHover: async (model, position) => {
			try {
				// Create a unique key for this hover request
				const requestKey = `${model.uri.toString()}:${position.lineNumber}:${position.column}`
				const now = Date.now()

				// Check if this is a duplicate request
				if (recentHoverRequests.has(requestKey)) {
					const lastRequestTime = recentHoverRequests.get(requestKey)!
				...
				// Record this request
				recentHoverRequests.set(requestKey, now)

				// Clean up old requests (keep map size manageable)
				if (recentHoverRequests.size > 100) {
					const oldThreshold = now - 5000 // 5 seconds
					for (const [key, timestamp] of recentHoverRequests.entries())
						if (timestamp < oldThreshold) recentHoverRequests.delete(key)
				}
				...
				// Replace Type aliases like Type$6 generated by tsup when
				// bundling .d.ts with standard Type alias displayed in editor
				...
				return {
					contents,
					range: {
						startLineNumber: model.getPositionAt(hoverInfo.textSpan.start)
							.lineNumber,
