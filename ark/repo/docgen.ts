...
const dtsBundleGen = () => {
	console.log(`✍️ Generating arktype DTS bundle for docs...`)

	shell(
		"pnpm tsup index.ts --dts-only --dts-resolve --format esm --out-dir .",
		{
			cwd: packagesByScope.type.path
		}
	)

	const expectedDtsBundlePath = join(packagesByScope.type.path, "index.d.ts")

	// Read DTS content instead of renaming the file
	try {
		arktypeRawDts = readFileSync(expectedDtsBundlePath, "utf8")
		console.log(`📚 Successfully read arktype DTS bundle.`)
	} catch (error) {
		console.error(`Error reading DTS bundle: ${error}`)
		process.exit(1)
	}
}

const writeApiDataFile = () => {
	writeFile(
		apiDataPath,
...
// prettier-ignore
export const arktypeRawDts = ${JSON.stringify(arktypeRawDts)}\n`
	)

	console.log(`📚 Successfully wrote API data to ${apiDataPath}.\n\n`)
}
