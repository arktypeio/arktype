...
contextualize(() => {
	it("root", () => {
		...
		attest(uuid("1234").toString()).equals('must be a UUID (was "1234")')
	})

	it("version subtype", () => {
		const uuidv4 = type("uuid.v4")

		attest(uuidv4(validUuidV4)).equals(validUuidV4)
		attest(uuidv4("1234").toString()).equals('must be a UUIDv4 (was "1234")')

		attest(ark.uuid.v5(validUuidV5)).equals(validUuidV5)
