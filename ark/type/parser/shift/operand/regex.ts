import type { Backslash, join, Scanner } from "@ark/util"
...
> =
	Scanner.shiftUntil<unscanned, Control> extends (
		Scanner.shiftResult<infer scanned, infer next>
	) ?
		next extends Scanner.shift<infer lookahead, infer nextUnscanned> ?
			lookahead extends Backslash ? {}
			: lookahead extends "|" ?
				parseSequence<
					nextUnscanned,
					inferred | `${join<sequence, "">}${scanned}`,
					[]
				>
			:	parseSequence<nextUnscanned, inferred, [...sequence, scanned]>
		:	inferred | `${join<sequence, "">}${scanned}`
	:	never
