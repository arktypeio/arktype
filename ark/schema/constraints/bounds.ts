...
> {
	...
	fold(into: FoldBranch<"minLength">) {
		if (
...
> {
	...
	fold(into: FoldBranch<"maxLength">) {
		if (
			into.basis?.domain !== "string" &&
			!into.basis?.extends(this.$.builtin.Array)
		) {
