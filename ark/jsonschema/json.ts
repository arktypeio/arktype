...
export const innerParseJsonSchema: Type<
	...
	any
> = JsonSchema.Schema.pipe(jsonSchema => {
	...
	if ("type" in jsonSchema) {
		let typeValidator: Type<unknown, any>

		switch (jsonSchema.type) {
			case "array":
				typeValidator = validateJsonSchemaArray.assert(jsonSchema) as never // A bug in ArkType makes this cast necessary
				break
			case "boolean":
			case "null":
			...
			case "integer":
			case "number":
				typeValidator = validateJsonSchemaNumber.assert(jsonSchema) as never // A bug in ArkType makes this cast necessary
				break
			case "object":
				typeValidator = validateJsonSchemaObject.assert(jsonSchema) as never // A bug in ArkType makes this cast necessary
				break
			case "string":
				typeValidator = validateJsonSchemaString.assert(jsonSchema) as never // A bug in ArkType makes this cast necessary
				break
			default:
				throwParseError(
