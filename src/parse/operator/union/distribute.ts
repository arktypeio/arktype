...
import { intersect, intersectKey } from "../../state/attributes/intersect.js"
import { traverseToDiscriminant } from "./prune.js"

const distribute = (attributes: Attributes, branches: AttributeBranches) => {
    if (branches[0] === "?") {
        ...
        if (caseAttributes) {
            return intersect(attributes, caseAttributes)
        } else {
            intersectKey(
                attributes,
                "contradiction",
