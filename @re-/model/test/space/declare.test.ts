...
describe("multifile", () => {
    it("compiles", () => {
        ...
        assert(space.group.type).type.toString.snap(
        ...
        assert(space.user.type.bestFriend).type.toString.snap(
        ...
        assert(
            space.$meta.model({ foozler: "user", choozler: "group[]" }).type
        ).type.toString.snap(
    ...
    it("single", () => {
        ...
        assert(space.$meta.model({ a: "gottaDefineThis" }).type).typed as {
            a: boolean
        }
    })
