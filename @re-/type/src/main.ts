...
export const parse: Node.ParseFn<string> = (def, ctx) =>
    loop(operand(new Parser.state(def), ctx), ctx)

export type Parse<Def extends string, Dict> = Loop<
    Base<Parser.State.New<Def>, Dict>,
    Dict
>
...
const next = (s: Parser.state, ctx: Node.context): Parser.state =>
    s.hasRoot() ? operator(s, ctx) : operand(s, ctx)
...
export const suffixLoop = (s: state<left.suffix>, ctx: node.Context): node => {
