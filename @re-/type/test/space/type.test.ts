...
describe("space", () => {
    it("single", () => {
        assert(space({ a: "string" }).$meta.infer.a).typed as string
        assert(() =>
            // @ts-expect-error
    ...
    it("independent", () => {
        assert(space({ a: "string", b: { c: "boolean" } }).$meta.infer.b)
            .typed as {
            c: boolean
        }
        assert(() =>
    ...
    it("interdependent", () => {
        assert(space({ a: "string", b: { c: "a" } }).$meta.infer.b.c)
            .typed as string
        assert(() =>
            // @ts-expect-error
    ...
    it("cyclic", () => {
        ...
        assert(cyclicSpace.$meta.infer.a).typed as {
            b: {
                a: {
                    b: {
                        a: any
        ...
        assert(cyclicSpace.$meta.infer.b.a.b.a.b.a.b.a).typed as {
            b: {
                a: any
            }
        }
        // @ts-expect-error
        assert(cyclicSpace.$meta.infer.a.b.a.b.c).type.errors.snap(
    ...
    it("cyclic eager", () => {
        ...
        assert(cyclicEagerSpace.$meta.infer.a).typed as {
            b: {
                a: {
                    b: {
                        a: any
    ...
    it("object list", () => {
        assert(space({ a: "string", b: [{ c: "a" }] }).$meta.infer.b).typed as [
            {
                c: string
            }
        ]
    ...
    it("doesn't try to validate any as a dictionary", () => {
        const parseWithAnySpace = space({} as any).$meta.type({
            literal: "string",
            // @ts-ignore
            alias: "myType"
        })
    ...
    it("doesn't try to validate any as a dictionary member", () => {
        assert(space({ a: {} as any }).$meta.type(["number", "a"]).infer)
            .typed as [number, any]
    })
    it("model from space", () => {
        const anotherCyclicSpace = space({ a: { b: "b" }, b: { a: "a" } })
        assert(anotherCyclicSpace.$meta.type("a|b|null").infer).typed as
            | { b: { a: { b: { a: any } } } }
            | { a: { b: { a: { b: any } } } }
            | null
        assert(() =>
            anotherCyclicSpace.$meta.type(
                // @ts-expect-error
                { nested: { a: "a", b: "b", c: "c" } },
                { parse: { eager: true } }
            )
