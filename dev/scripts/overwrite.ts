...
const readFile = (filePath: string, onSuccess: (data: string) => void): void => {
...
const replaceAll
  ...
  = (matchers) =>
    (input) => {
      ...
      if (input.includes(`utils/src`) || input.includes(`attest/src`) && input !== out) {
 ...
 */
const traverse
  : (ignorePaths: readonly string[]) => (rootPath: string) => readonly string[]
  = (ignorePaths) =>
    (rootPath) => {
      const go = (parentPath: string, acc: readonly string[]): readonly string[] =>
 ...
 */
const overwrite
  : (matchers: Matchers) => (files: readonly string[]) => void
  = (matchers) =>
    (files) =>
      void files.map(
        file =>
...
export const rewritePaths
  : (ignorePaths: readonly string[], matchDictionary: Record<string, string>) => (buildPath: string) => void
  = (ignorePaths, matchDictionary) =>
    (buildPath) => {
